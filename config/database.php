...
$pgsqlDb = env('DB_DATABASE', 'forge');
$pgsqlUser = env('DB_USERNAME', 'forge');
$pgsqlPass = env('DB_PASSWORD', '');
$pgsqlSsl = 'prefer';

if ($parsedUrl) {
    $pgsqlHost = $parsedUrl['host'] ?? $pgsqlHost;
    $pgsqlPort = $parsedUrl['port'] ?? $pgsqlPort;
    $pgsqlDb = isset($parsedUrl['path']) ? ltrim($parsedUrl['path'], '/') : $pgsqlDb;
    $pgsqlUser = $parsedUrl['user'] ?? $pgsqlUser;
    $pgsqlPass = $parsedUrl['pass'] ?? $pgsqlPass;

    if (isset($parsedUrl['query'])) {
        parse_str($parsedUrl['query'], $queryParams);
        if (isset($queryParams['sslmode'])) {
            $pgsqlSsl = $queryParams['sslmode'];
        }
    }

    if (str_contains($pgsqlHost, 'neon.tech')) {
        $hostParts = explode('.', $pgsqlHost);
        $endpointId = $hostParts[0];
        if (!str_contains($pgsqlHost, 'options=')) {
            $pgsqlHost .= ";options='endpoint={$endpointId}'";
        }
    }
}
